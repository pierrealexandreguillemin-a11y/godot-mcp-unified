...
import { getAllToolDefinitions, getRegisteredToolNames, getToolHandler, isToolRegistered, } from '../tools/ToolRegistry.js';
    ...
    try {
        ...
        for (const definition of toolDefinitions) {
            ...
            if (!definition.description || typeof definition.description !== 'string') {
                throw new Error(`Invalid tool definition for ${definition.name}: missing or invalid description`);
            }
            if (!definition.inputSchema || typeof definition.inputSchema !== 'object') {
                throw new Error(`Invalid tool definition for ${definition.name}: missing or invalid inputSchema`);
            }
            if (!definition.inputSchema.properties ||
                typeof definition.inputSchema.properties !== 'object') {
                throw new Error(`Invalid tool definition for ${definition.name}: missing or invalid properties`);
            }
            if (!Array.isArray(definition.inputSchema.required)) {
                throw new Error(`Invalid tool definition for ${definition.name}: missing or invalid required array`);
            }
        }
        ...
        for (const tool of toolsForAgent) {
            ...
            if (!tool.description)
                throw new Error(`Tool ${tool.name} missing description field`);
            if (!tool.inputSchema)
                throw new Error(`Tool ${tool.name} missing inputSchema field`);
        }
